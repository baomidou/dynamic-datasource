...
import static com.baomidou.dynamic.datasource.support.DdConstants.XA_DRUID_DATASOURCE;

/**
 * DruidXADataSourceCreator数据源创建器
 *
 * @author TaoYu
 * @since 2020/1/21
 */
@Slf4j
public class DruidXADataSourceCreator extends AbstractDataSourceCreator implements DataSourceCreator, InitializingBean {
    ...
    public DataSource doCreateDataSource(DataSourceProperty dataSourceProperty) {
        log.error("\n 执行 >>>>>>> DruidXADataSourceCreator  --- 数据源创建器   <<<<<<<<");
        ...
        if (Boolean.FALSE.equals(dataSourceProperty.getLazy())) {
            try {
                ...
                throw new ErrorCreateDataSourceException("DruidXADataSource create error", e);
            }
        }

        String uniqueDBNameStr = dataSourceProperty.getUrl().split("\\?",0)[0];
        uniqueDBNameStr = uniqueDBNameStr.split("/")[3];
        AtomikosDataSourceBean atomikosDataSourceBean = new AtomikosDataSourceBean();
        atomikosDataSourceBean.setXaDataSourceClassName("com.alibaba.druid.pool.xa.DruidXADataSource");
        atomikosDataSourceBean.setUniqueResourceName(uniqueDBNameStr+"-XA");
    ...
    public boolean support(DataSourceProperty dataSourceProperty) {
        ...
        return type == null || XA_DRUID_DATASOURCE.equals(type.getName());
    }
